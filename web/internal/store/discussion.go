...
// DiscussionStore 负责题目评论 / 题解评论 / 比赛评论。
type DiscussionStore struct{ db *DB }

func NewDiscussionStore(db *DB) *DiscussionStore { return &DiscussionStore{db: db} }

// ListByProblem 列出某题的评论(含作者名,父评论在前)。
func (s *DiscussionStore) ListByProblem(ctx context.Context, problemID string) ([]model.DiscussionPost, error) {
	return s.list(ctx, "problem_id = $1", problemID)
}

// ListByEditorial 列出某题解的评论。
func (s *DiscussionStore) ListByEditorial(ctx context.Context, editorialID string) ([]model.DiscussionPost, error) {
	return s.list(ctx, "editorial_id = $1", editorialID)
}

func (s *DiscussionStore) list(ctx context.Context, where string, arg string) ([]model.DiscussionPost, error) {
	...
	list := []model.DiscussionPost{}
	for rows.Next() {
		var p model.DiscussionPost
...
func (s *DiscussionStore) CreateProblemPost(ctx context.Context, problemID, authorID, contentMD string, parentID *int64) (*model.DiscussionPost, error) {
	var p model.DiscussionPost
...
func (s *DiscussionStore) CreateEditorialPost(ctx context.Context, editorialID, authorID, contentMD string) (*model.DiscussionPost, error) {
	var p model.DiscussionPost
	err := s.db.Pool.QueryRow(ctx,
...
func (s *DiscussionStore) Delete(ctx context.Context, postID int64) error {
	_, err := s.db.Pool.Exec(ctx, `DELETE FROM discussion_posts WHERE id = $1`, postID)
	return err
}
