...
`web` 是 Vertex 的业务与持久化边界。它提供面向浏览器的 REST API、面向 Judge 的内部长轮询协议、认证会话、数据库迁移和 Swagger/OpenAPI 文档。

Go module：`github.com/RimuruChan/Vertex/web`
...
- Judge 使用 service token 调用 `/internal/judge/v1`，不直接连接数据库。
...
以上 Go 命令在 `web/` 目录执行。服务默认监听 `:8080`：
...
该命令更新 `docs/docs.go`、`docs/swagger.json` 和 `docs/swagger.yaml`。修改 handler、路由或 DTO 后必须提交生成结果；前端再从 `swagger.json` 生成客户端。
