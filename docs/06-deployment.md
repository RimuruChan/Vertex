...
出题检查的 TeX 渲染需要镜像中预装的 XeTeX、常用 LaTeX 宏包与 Noto CJK 字体。Dockerfile 将预生成格式和索引放入 `/usr/share/vertex/texmf-var`，避免为 TeX 开放整个 `/var`；`statement-toolchain.txt` 记录发行包版本并参与工具链指纹。题面独立沙箱最多使用 1 GiB 内存、8 个进程、30 秒 CPU / 40 秒墙钟及默认 64 MiB 工作区，PDF 上限 32 MiB；容器总预算应覆盖出题检查与评测并发。运行时不下载宏包。当前检查协议为 `vertex-authoring-3`，更旧 Worker 不领取新协议任务。
...
- 当前开发阶段的 schema 合并在 `000001_init`。旧本地库与新基线不一致时，创建独立开发/测试库并保留原库；工具不会自动重置业务数据。公告置顶到期由查询时的截止时间判断，不需要部署定时清理任务，也不会删除或下架公告。
- 备份:卷 `pgdata`(全量)+ `testdata`(测试数据)。测试数据体积大,可与 DB 分开备份。

## 8. 出题内容回收

Server 在启动后及固定间隔执行有界回收，不需要另配 cron。配置使用 Go duration 格式：

| 变量 | 默认值 | 含义 |
|---|---|---|
| `AUTHORING_GC_INTERVAL` | `1h` | 回收轮次间隔 |
| `AUTHORING_GC_GRACE` | `24h` | 未附着上传和无引用对象的宽限期，最少 `1h` |

提交、当前副本、合并结果、检查输入、有效导入预检和发布版本引用的材料保留；检查/发布/候选数据引用的产物也保留。未附着到材料树的上传授权是临时的，重复上传同一内容会刷新期限。宽限期按记录创建/上传刷新或文件修改时间计算。删除题目后，已经足够旧且不再被任何记录引用的内容可在下一轮清理。

每个题目的存储写入与回收通过 PostgreSQL 锁互斥；繁忙题目本轮跳过。数据库引用清理先提交，之后才删除文件；目录先原子改名再递归清理，因此中断后的残留不会被当作完整产物复用。回收仅识别内部 UUID 命名空间、摘要对象和本版本创建的暂存目录，不跟随符号链接，不删除未知文件。不要在这些内部目录存放手工备份。

有清理结果时日志记录对象数、字节数和已删除的引用记录；失败保留数据供后续重试。备份应同时包含数据库及整个 `testdata` 卷，其中 `.authoring` 存放内容树对应的原始材料。
