...
比赛基本设置中的「奖牌设置」支持关闭、固定人数和有效人数占比。有效人数是当前榜单中至少通过一道题的选手人数；只有这些选手参与奖牌分配。

新建 ICPC 比赛和应用 ICPC 预设时，默认按有效人数设置金牌 10%、银牌 20%、铜牌 30%；其他赛制预设默认关闭奖牌。更新比赛时省略奖牌配置保留原值，显式关闭也不会被默认值覆盖。

- 固定人数分别填写金、银、铜牌名额，0 表示该档没有名额。
- 百分比分别计算 `floor(有效人数 × 比例 / 100)`，三档比例总和不得超过 100%。
- 按有效选手的排名依次划分金、银、铜档，使用累计名额边界。同排名同奖牌，跨边界的并列组归入较高档，因此该档实际人数可能超过设置值，并占用后续档位的名额。
- 公开封榜只按可见的通过情况计算人数和奖牌；内部榜使用实时成绩。解榜、重测和奖牌配置改变后，下次读取榜单时重新计算，不保存过期的获奖结果。
- `contests.medal_mode`、`medal_gold`、`medal_silver`、`medal_bronze` 直接维护在 `000001_init`。接口通过 `medals` 对象读写配置，更新时省略该对象保留已有配置。榜单返回各行 `medal` 和实际分配人数汇总。

## 11. 已知边界

- **子任务计分**：出题内容树可保留测试点分组与分值，但提交评测尚未接入子任务聚合。这类材料允许编辑和检查，发布检查会明确阻止发布，避免把部分分规则错误地当作普通通过/失败题。IOI 比赛按评测返回的整体 score 排名。
