...
constexpr std::uint64_t kDefaultWorkspaceBytes = 64ULL * 1024 * 1024;
constexpr std::uint64_t kDefaultWorkspaceInodes = 4096;
...
struct Options {
    ...
    std::uint64_t workspace_bytes = kDefaultWorkspaceBytes;
    std::uint64_t workspace_inodes = kDefaultWorkspaceInodes;
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        } else if (arg == "--time-hard-ms") {
            options.time_hard_ms =
                parse_uint(require_value(argc, argv, i, arg), "hard time limit");
        } else if (arg == "--wall-ms") {
            options.wall_ms = parse_uint(require_value(argc, argv, i, arg), "wall limit");
        } else if (arg == "--wall-hard-ms") {
            options.wall_hard_ms =
                parse_uint(require_value(argc, argv, i, arg), "hard wall limit");
        ...
        } else if (arg == "--workspace-bytes") {
            options.workspace_bytes =
                parse_uint(require_value(argc, argv, i, arg), "workspace byte limit");
        } else if (arg == "--workspace-inodes") {
            options.workspace_inodes =
                parse_uint(require_value(argc, argv, i, arg), "workspace inode limit");
    ...
    if (options.action == "run") {
        ...
        (void)checked_multiply(options.time_hard_ms, 1000, "hard time limit");
        (void)checked_multiply(options.wall_ms, 1000, "wall limit");
        (void)checked_multiply(options.wall_hard_ms, 1000, "hard wall limit");
    ...
    for (const char character : options.cpu_set) {
        if ((character < '0' || character > '9') && character != ',' && character != '-') {
            throw Error("invalid CPU set: " + options.cpu_set);
        }
    }
    return options;
}
...
void require_cpu_set_available(const Options& options) {
    if (options.cpu_set.empty()) {
        return;
    }
    const fs::path root = cgroup_root();
    if (!file_has_word(root / "cgroup.controllers", "cpuset") ||
        !file_has_word(root / "cgroup.subtree_control", "cpuset")) {
        throw Error("configured CPU set requires a delegated cpuset controller");
    }
    (void)read_word_file(root / "cpuset.mems.effective");
}
...
void create_cgroup(const Options& options) {
    ...
    try {
        if (!options.cpu_set.empty()) {
            require_cpu_set_available(options);
            if (!fs::exists(group / "cpuset.mems") ||
                !fs::exists(group / "cpuset.cpus")) {
                throw Error("cpuset controller is not active for child cgroups");
            }
            write_text(group / "cpuset.mems",
                       read_word_file(root / "cpuset.mems.effective"));
            write_text(group / "cpuset.cpus", options.cpu_set);
        }
...
WorkspaceUsage scan_workspace(const Options& options) {
    const Fd root(open(work_dir(options).c_str(),
                       O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW));
    if (root.get() < 0) {
...
bool update_workspace_stats(const Options& options, RunStats& stats) {
    const WorkspaceUsage usage = scan_workspace(options);
...
RunStats supervise(const Options& options, pid_t child, int error_fd,
    ...
    while (!stats.child_reaped) {
        ...
        if (update_workspace_stats(options, stats) && !stats.workspace_killed) {
