...
`sandbox` 是 Vertex 的原生 C++ 隔离执行器源码。它负责启动不受信任程序、施加 Linux 安全与资源策略，并把退出状态和资源统计写入结构化 meta 文件。
...
冒烟测试覆盖文件和网络隔离、soft/hard timeout、内存/输出限制、workspace bytes/inodes、信号与退出状态映射。
