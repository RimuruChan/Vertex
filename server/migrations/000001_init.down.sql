-- Reverse the baseline in dependency order. No CASCADE: missing dependencies
-- should fail the rollback instead of silently removing unrelated objects.

-- Announcements and community
DROP TABLE IF EXISTS announcements;
DROP TABLE IF EXISTS discussion_posts;
DROP TABLE IF EXISTS editorial_votes;
DROP TABLE IF EXISTS editorials;
-- Problem sets
DROP TABLE IF EXISTS problem_set_problems;
DROP TABLE IF EXISTS problem_set_access;
DROP TABLE IF EXISTS problem_sets;
-- Judging and submissions
DROP TABLE IF EXISTS rejudging_submissions;
DROP TABLE IF EXISTS rejudgings;
DROP FUNCTION IF EXISTS visible_submissions(UUID,TEXT,BOOLEAN,BOOLEAN,BOOLEAN,TIMESTAMPTZ);
DROP VIEW IF EXISTS submission_results;
DROP TABLE IF EXISTS judge_jobs;
DROP FUNCTION IF EXISTS protect_judge_job_input();
ALTER TABLE IF EXISTS submissions DROP CONSTRAINT IF EXISTS submissions_result;
ALTER TABLE IF EXISTS submissions DROP CONSTRAINT IF EXISTS submissions_generation;
DROP TABLE IF EXISTS judgements;
DROP FUNCTION IF EXISTS valid_judgement_cases(JSONB);
DROP FUNCTION IF EXISTS protect_judgement_input();
DROP TABLE IF EXISTS submissions;
-- Contests
DROP TABLE IF EXISTS clarifications;
DROP TABLE IF EXISTS contest_submission_cells;
DROP VIEW IF EXISTS contest_staff;
DROP TABLE IF EXISTS contest_access;
DROP TABLE IF EXISTS contest_participants;
DROP TABLE IF EXISTS contest_problems;
DROP FUNCTION IF EXISTS pin_contest_problem_version();
DROP TABLE IF EXISTS contests;
-- Authoring and immutable problem releases
ALTER TABLE IF EXISTS problem_versions DROP CONSTRAINT IF EXISTS problem_versions_source_check;
ALTER TABLE IF EXISTS problem_versions DROP CONSTRAINT IF EXISTS problem_versions_source_commit;
DROP TABLE IF EXISTS problem_build_jobs;
DROP TABLE IF EXISTS problem_version_files;
DROP FUNCTION IF EXISTS protect_problem_build_input();
DROP TABLE IF EXISTS problem_merge_sessions;
DROP TABLE IF EXISTS problem_imports;
DROP TABLE IF EXISTS problem_working_copies;
DROP TABLE IF EXISTS problem_authoring_heads;
DROP TABLE IF EXISTS problem_commits;
DROP TABLE IF EXISTS problem_tree_blobs;
DROP TABLE IF EXISTS problem_content_trees;
DROP TABLE IF EXISTS problem_blob_uploads;
DROP TABLE IF EXISTS problem_blobs;
DROP FUNCTION IF EXISTS protect_authoring_content();
DROP TABLE IF EXISTS problem_origins;
DROP FUNCTION IF EXISTS protect_problem_origin();
ALTER TABLE IF EXISTS problems DROP CONSTRAINT IF EXISTS problems_published_version;
DROP TABLE IF EXISTS problem_versions;
DROP FUNCTION IF EXISTS protect_problem_release();
DROP TABLE IF EXISTS problem_tags;
DROP TABLE IF EXISTS tags;
DROP TABLE IF EXISTS problem_access;
DROP TABLE IF EXISTS problems;
-- Domains and membership (break the owner/member cycle explicitly)
DROP TABLE IF EXISTS domain_audit_events;
DROP TABLE IF EXISTS domain_group_members;
DROP TABLE IF EXISTS domain_groups;
DROP TABLE IF EXISTS domain_number_counters;
DROP FUNCTION IF EXISTS allocate_domain_number();
DROP FUNCTION IF EXISTS protect_resource_identity();
ALTER TABLE IF EXISTS domains DROP CONSTRAINT IF EXISTS domains_owner_membership;
DROP TABLE IF EXISTS domain_members;
DROP TABLE IF EXISTS domain_roles;
DROP TABLE IF EXISTS domains;
-- Identity and extensions
DROP TABLE IF EXISTS auth_sessions;
DROP TABLE IF EXISTS users;
DROP EXTENSION IF EXISTS pgcrypto;
