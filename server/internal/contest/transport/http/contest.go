...
func (h *ContestHandler) writeError(c *gin.Context, err error, fallback string) {
	...
	case errors.Is(err, contestdomain.ErrNotFound), errors.Is(err, contestdomain.ErrProblemNotInContest):
		writeAPIError(c, http.StatusNotFound, "contest.not_found", "contest not found")
	case errors.Is(err, contestdomain.ErrForbidden):
		writeAPIError(c, http.StatusForbidden, "contest.forbidden", "insufficient contest permissions")
	case errors.Is(err, contestdomain.ErrVersionConflict):
		writeAPIError(c, http.StatusConflict, "contest.version_conflict", err.Error())
	case errors.Is(err, contestdomain.ErrRegistrationClosed):
		writeAPIError(c, http.StatusBadRequest, "contest.registration_closed", err.Error())
	case errors.Is(err, contestdomain.ErrSelfRegistrationDisabled):
		writeAPIError(c, http.StatusForbidden, "contest.self_registration_disabled", err.Error())
	case errors.Is(err, contestdomain.ErrInvalidPassword):
		writeAPIError(c, http.StatusForbidden, "contest.invalid_password", err.Error())
	case errors.Is(err, contestdomain.ErrRegistrationNeeded):
		writeAPIError(c, http.StatusForbidden, "contest.registration_required", err.Error())
	case errors.Is(err, contestdomain.ErrRankboardHidden):
		writeAPIError(c, http.StatusForbidden, "contest.rankboard_hidden", err.Error())
	case errors.Is(err, contestdomain.ErrNotParticipant):
		writeAPIError(c, http.StatusForbidden, "contest.not_participant", err.Error())
	case errors.Is(err, contestdomain.ErrClarificationClosed):
		writeAPIError(c, http.StatusForbidden, "contest.clarifications_closed", err.Error())
	case errors.Is(err, contestdomain.ErrClarificationNotFound):
...
func contestInput(request dto.ContestUpsertRequest) contestdomain.UpsertInput {
