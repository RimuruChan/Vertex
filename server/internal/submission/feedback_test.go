package submission_test

import (
	"github.com/RimuruChan/Vertex/server/internal/contest"
	submissionapp "github.com/RimuruChan/Vertex/server/internal/submission"
	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
)

func judged() *submissionapp.Submission {
	return &submissionapp.Submission{
		ID: "submission-1", Status: "Wrong Answer", Score: 40,
		TotalTimeMs: 120, PeakMemoryKb: 2048, CompileResult: "warning: unused",
		JudgedCases: 5, TotalCases: 10,
		CaseResults: []submissionapp.CaseResult{{CaseIndex: 1, Verdict: "Accepted"}},
	}
}

var _ = Describe("Redact", func() {
	It("leaves everything intact at full feedback", func() {
		item := judged()
		submissionapp.Redact(item, contest.FeedbackFull)
		Expect(item.Status).To(Equal("Wrong Answer"))
		Expect(item.CaseResults).To(HaveLen(1))
		Expect(item.TotalTimeMs).To(Equal(120))
	})

	It("keeps the verdict but drops test detail at summary feedback", func() {
		item := judged()
		submissionapp.Redact(item, contest.FeedbackSummary)
		Expect(item.Status).To(Equal("Wrong Answer"))
		Expect(item.CaseResults).To(BeEmpty())
		Expect(item.CompileResult).To(BeEmpty())
		Expect(item.TotalTimeMs).To(Equal(0))
		Expect(item.PeakMemoryKb).To(Equal(0))
		// Progress counters would leak how far the hidden test set got.
		Expect(item.TotalCases).To(Equal(0))
	})

	It("hides the verdict entirely at no feedback", func() {
		item := judged()
		submissionapp.Redact(item, contest.FeedbackNone)
		Expect(item.Status).To(Equal(submissionapp.HiddenStatus))
		Expect(item.Score).To(Equal(0))
		Expect(item.CaseResults).To(BeEmpty())
	})

	It("still shows a queued submission as queued at no feedback", func() {
		// A contestant must be able to tell their submission arrived, even in
		// a contest that reveals nothing about the result.
		for _, status := range []string{"Pending", "Judging"} {
			item := judged()
			item.Status = status
			submissionapp.Redact(item, contest.FeedbackNone)
			Expect(item.Status).To(Equal(status))
		}
	})

	It("never removes the identity of the submission itself", func() {
		item := judged()
		item.SourceCode = "int main(){}"
		submissionapp.Redact(item, contest.FeedbackNone)
		Expect(item.ID).To(Equal("submission-1"))
		Expect(item.SourceCode).To(Equal("int main(){}"))
	})
})

var _ = Describe("RejudgeSelector", func() {
	It("treats a selector with no restriction as empty", func() {
		Expect(submissionapp.RejudgeSelector{}.IsEmpty()).To(BeTrue())
		Expect(submissionapp.RejudgeSelector{Reason: "oops"}.IsEmpty()).To(BeTrue())
	})

	It("accepts any single restriction", func() {
		Expect(submissionapp.RejudgeSelector{ContestID: "c"}.IsEmpty()).To(BeFalse())
		Expect(submissionapp.RejudgeSelector{ProblemID: "p"}.IsEmpty()).To(BeFalse())
		Expect(submissionapp.RejudgeSelector{Status: "Accepted"}.IsEmpty()).To(BeFalse())
		Expect(submissionapp.RejudgeSelector{SubmissionIDs: []string{"s"}}.IsEmpty()).To(BeFalse())
	})
})

var _ = Describe("Rejudging progress", func() {
	It("is finished once every member has been re-judged", func() {
		batch := submissionapp.Rejudging{TotalCount: 3, DoneCount: 2}
		Expect(batch.Finished()).To(BeFalse())
		batch.DoneCount = 3
		Expect(batch.Finished()).To(BeTrue())
	})
})
