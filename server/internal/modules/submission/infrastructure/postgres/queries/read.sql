			  ...
			  AND (
				...
				OR (
					...
					AND (
						...
						OR (c.visibility IN ('password','private') AND (c.visibility='password' OR (sqlc.arg(can_submit)::boolean AND sqlc.arg(active_member)::boolean AND (c.admission='members' OR EXISTS(SELECT 1 FROM contest_access a WHERE a.contest_id=c.id AND a.role='participant' AND (a.user_id=NULLIF(sqlc.arg(viewer_id)::text,'')::uuid OR a.group_id IN(SELECT group_id FROM domain_group_members WHERE domain_id=c.domain_id AND user_id=NULLIF(sqlc.arg(viewer_id)::text,'')::uuid)))))) AND EXISTS (
							SELECT 1 FROM contest_participants participant
							WHERE participant.contest_id = c.id
							  AND participant.user_id = NULLIF(sqlc.arg(viewer_id)::text,'')::uuid
						))
