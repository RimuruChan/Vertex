...
SELECT p.id,w.title,w.time_limit_ms,w.memory_limit_kb,w.judge_type,w.package_revision,p.domain_id,w.data_revision
...
SELECT problem.id, problem.public_id, w.title, problem.visibility, w.judge_type,
		        ...
		        w.package_revision, COALESCE(testdata.data_revision,0)::integer AS built_revision, w.last_built_at,
		        COALESCE(testdata.case_count, 0), COALESCE(testdata.checker, ''),
		        COALESCE(testdata.data_version, 0), COALESCE(testdata.sha256, ''), w.data_revision AS data_revision,
		 ...
		 LEFT JOIN problem_candidates AS testdata ON testdata.problem_id = problem.id
