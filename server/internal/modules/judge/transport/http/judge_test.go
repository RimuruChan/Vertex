...
import (
	...
	dto "github.com/RimuruChan/Vertex/server/internal/modules/judge/transport/http/dto"
...
var _ = Describe("JudgeHandler", func() {
	...
	It("returns the immutable claim snapshot", func() {
		service := &fakeJudgeService{job: &judgedomain.Job{
	...
	It("maps stale result fencing to conflict", func() {
		service := &fakeJudgeService{completeErr: judgedomain.ErrStaleLease}
		handler := NewJudgeHandler(service)
		router := gin.New()
	...
	It("maps invalid worker identities to a bad request", func() {
		service := &fakeJudgeService{claimErr: judgeapp.ErrInvalidWorker}
		handler := NewJudgeHandler(service)
		router := gin.New()
		router.POST("/jobs/claim", handler.Claim)
		response := httptest.NewRecorder()
...
func (f *fakeJudgeService) Claim(_ context.Context, workerID string, wait time.Duration) (*judgedomain.Job, error) {
...
func (f *fakeJudgeService) Complete(_ context.Context, result judgedomain.Result) error {
	f.result = result
