...
func icpcRules() contestdomain.ScoringRules {
	return contestdomain.ScoringRules{
		Format: contestdomain.FormatICPC, BeginAt: contestStart, EndAt: at(300),
		PenaltyMinutes: 20, PenalizeCompileError: true, MaxPoints: 100,
	}
}

func submission(minutes int, status string, score int) contestdomain.ScoredSubmission {
	return contestdomain.ScoredSubmission{SubmittedAt: at(minutes), Status: status, Score: score}
}

var _ = Describe("ScoreCell ICPC", func() {
	It("charges solve time plus a penalty for each earlier rejection", func() {
		cell := contestdomain.ScoreCell(icpcRules(), []contestdomain.ScoredSubmission{
...
var _ = Describe("ScoreCell freeze split", func() {
	frozenRules := func() contestdomain.ScoringRules {
		rules := icpcRules()
		freeze := at(240)
...
var _ = Describe("ScoreCell IOI", func() {
	ioiRules := func() contestdomain.ScoringRules {
		rules := icpcRules()
		rules.Format = contestdomain.FormatIOI
		return rules
	}

	It("keeps the best score even when a later submission is worse", func() {
		cell := contestdomain.ScoreCell(ioiRules(), []contestdomain.ScoredSubmission{
...
var _ = Describe("ScoreCell OI", func() {
	oiRules := func() contestdomain.ScoringRules {
		rules := icpcRules()
		rules.Format = contestdomain.FormatOI
		return rules
	}

	It("scores only the last submission, even when it is worse", func() {
		cell := contestdomain.ScoreCell(oiRules(), []contestdomain.ScoredSubmission{
...
var _ = Describe("Totals", func() {
	...
	It("sums the public view for a frozen board", func() {
		...
		totals := contestdomain.Totals(cells, false)
	...
	It("tracks the latest accepted time for score tie-breaking", func() {
		...
		totals := contestdomain.Totals([]contestdomain.Cell{
...
var _ = Describe("AssignRanks", func() {
	...
	It("ranks IOI by score and breaks ties on who got there first", func() {
		...
		contestdomain.AssignRanks(contestdomain.FormatIOI, rows)
...
var _ = Describe("Contest phase helpers", func() {
	...
	It("restores full feedback once the contest ends", func() {
		item := build()
		Expect(item.FeedbackFor(at(100))).To(Equal(contestdomain.FeedbackNone))
		Expect(item.FeedbackFor(at(301))).To(Equal(contestdomain.FeedbackFull))
	})
})

func TestScoring(t *testing.T) { RegisterFailHandler(Fail); RunSpecs(t, "Contest rules") }
