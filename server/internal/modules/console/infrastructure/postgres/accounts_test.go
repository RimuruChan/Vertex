...
var _ = Describe("Console store against PostgreSQL", func() {
	...
	It("aggregates the dashboard across domains", func(ctx SpecContext) {
		...
		stats, err = store.Stats(ctx)
		Expect(err).NotTo(HaveOccurred())
		Expect(stats.Problems).To(Equal(2))
		Expect(stats.PublicProblems).To(Equal(1))
	...
	It("blocks an account and revokes its sessions in one step", func(ctx SpecContext) {
		...
		disabled := true
		updated, err := store.UpdateAccount(ctx, member, consoledomain.AccountUpdate{
		...
		disabled = false
		updated, err = store.UpdateAccount(ctx, member, consoledomain.AccountUpdate{Disabled: &disabled})
		Expect(err).NotTo(HaveOccurred())
	...
	It("changes only the fields the caller set", func(ctx SpecContext) {
		...
		rating := 1500
		updated, err := store.UpdateAccount(ctx, member, consoledomain.AccountUpdate{Rating: &rating})
		Expect(err).NotTo(HaveOccurred())
	...
	It("hides unpublished announcements from readers and pins the rest", func(ctx SpecContext) {
		...
		Expect(err).NotTo(HaveOccurred())
		draft, err := store.CreateAnnouncement(manager(ctx), admin, consoledomain.AnnouncementInput{
		...
		Expect(store.DeleteAnnouncement(manager(ctx), draft.ID)).To(MatchError(consoledomain.ErrNotFound))
	})
})
