...
func (s *ProblemStore) UserStatuses(ctx context.Context, viewerID string, problemIDs []string) (map[string]string, error) {
	...
	rows, err := s.db.Pool.QueryContext(ctx,
		`SELECT problem_id, bool_or(status = 'Accepted') AS solved
		 FROM submissions
		 WHERE user_id = $1::uuid AND problem_id = ANY($2) AND contest_id IS NULL AND domain_id = $3
		 GROUP BY problem_id`, viewerID, problemIDs, domain.ID(ctx))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *ProblemStore) fillTags(ctx context.Context, problems []Problem) error {
	...
	rows, err := s.db.Pool.QueryContext(ctx,
		`SELECT pt.problem_id, t.name
		 ...
		 WHERE pt.problem_id = ANY($1) AND pt.domain_id = $2 ORDER BY t.name`, ids, domain.ID(ctx))
	if err != nil {
		return err
	}
	defer rows.Close()
