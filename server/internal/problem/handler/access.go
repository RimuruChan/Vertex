...
func (h *AdminProblemHandler) SetGrant(c *gin.Context) {
	...
	err := h.service.SetGrant(c.Request.Context(), c.Param("id"), problem.GrantInput{Username: request.Username, Group: request.Group, Role: problem.AccessRole(request.Role)})
	if err != nil {
		writeProblemError(c, err)
		return
	}
