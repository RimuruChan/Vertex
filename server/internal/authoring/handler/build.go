...
type BuildService interface {
	Claim(ctx context.Context, workerID string, wait time.Duration) (*authoring.Build, *authoring.Package, error)
	Progress(ctx context.Context, progress authoring.Progress) error
	UploadPackage(ctx context.Context, buildID, problemID, workerID, leaseToken string, archive []byte) (*authoring.PackageUpload, error)
	Complete(ctx context.Context, result authoring.BuildResult, checker string) error
}
...
func (h *BuildHandler) Claim(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, authoring.ErrInvalidInput) {
...
func (h *BuildHandler) UploadPackage(c *gin.Context) {
	c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, authoring.MaxPackageBytes)
...
func writeBuildError(c *gin.Context, err error) {
	...
	case errors.Is(err, authoring.ErrStaleLease):
		httpx.WriteError(c, http.StatusConflict, "build.stale_lease", authoring.ErrStaleLease.Error())
	...
	case errors.Is(err, authoring.ErrPackageTarget):
