...
type PackageHandler struct{ service *authoring.Service }

func NewPackageHandler(service *authoring.Service) *PackageHandler {
...
func (h *PackageHandler) Templates(c *gin.Context) {
	templates := dto.FromTemplates(authoring.Templates())
...
func (h *PackageHandler) StartBuild(c *gin.Context) {
	...
	if errors.Is(err, authoring.ErrBuildRunning) && build != nil {
...
func writeAuthoringError(c *gin.Context, err error) {
	...
	case errors.Is(err, authoring.ErrNotBuildable):
