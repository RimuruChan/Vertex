...
var _ = Describe("Explicit releases against PostgreSQL", func() {
	...
	as := func(ctx context.Context, user string) context.Context {
		return domain.WithScope(ctx, domain.Scope{Domain: domain.Domain{ID: domain.OfficialID}, UserID: user})
	}
	BeforeEach(func(ctx SpecContext) {
		...
		users := identity.NewUserStore(integrationDB)
		...
		item, err = writer.Create(as(ctx, owner), owner, &problem.CreateInput{Title: "Initial", StatementMD: "Initial statement", Visibility: "private"})
		Expect(err).NotTo(HaveOccurred())
		Expect(writer.SetGrant(as(ctx, owner), item.ID, problem.GrantInput{Username: "editor", Role: problem.AccessEditor})).To(Succeed())
	})
	It("separates working statements and uploaded candidates from explicit publication", func(ctx SpecContext) {
		_, err := packages.SaveStatement(as(ctx, editor), Statement{ProblemID: item.ID, Language: "zh", Name: "Draft name", Legend: "Draft statement"})
		Expect(err).NotTo(HaveOccurred())
		...
		input := PublishInput{Revision: meta.PackageRevision, ArtifactVersion: meta.TestdataVersion}
		...
		_, err = packages.SaveStatement(as(ctx, editor), Statement{ProblemID: item.ID, Language: "zh", Name: "Second name", Legend: "Second statement"})
		Expect(err).NotTo(HaveOccurred())
		_, err = packages.Publish(as(ctx, owner), item.ID, input)
		Expect(err).To(MatchError(ErrRevisionConflict))
		meta, err = packages.Meta(as(ctx, owner), item.ID)
		Expect(err).NotTo(HaveOccurred())
		Expect(meta.DataRevision).To(Equal(meta.BuiltRevision))
		release, err = packages.Publish(as(ctx, owner), item.ID, PublishInput{Revision: meta.PackageRevision, ArtifactVersion: meta.TestdataVersion})
		Expect(err).NotTo(HaveOccurred())
	...
	It("seals build inputs before queueing and does not reuse an old attempt's artifact", func(ctx SpecContext) {
		file, err := packages.SaveFile(as(ctx, editor), File{ProblemID: item.ID, Kind: KindSolution, Name: "main.cpp", Language: "cpp", SourceCode: "original", IsActive: true})
		Expect(err).NotTo(HaveOccurred())
		_, err = packages.CreateTest(as(ctx, editor), Test{ProblemID: item.ID, Source: TestManual, InputData: "1\n"})
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(builds.RecordPackage(ctx, first.ID, item.ID, first.WorkerID, first.LeaseToken, PackageUpload{StoragePath: path.Join(item.ID, "oldhash"), SHA256: "oldhash", CaseCount: 1, Checker: "diff"})).To(Succeed())
		...
		Expect(builds.Complete(ctx, BuildResult{BuildID: second.ID, WorkerID: second.WorkerID, LeaseToken: second.LeaseToken, Success: true}, "diff")).To(Succeed())
	...
	It("keeps a queued judge generation on its submitted release after a new publication", func(ctx SpecContext) {
		...
		meta, err := packages.Meta(as(ctx, owner), item.ID)
		Expect(err).NotTo(HaveOccurred())
		first, err := packages.Publish(as(ctx, owner), item.ID, PublishInput{Revision: meta.PackageRevision, ArtifactVersion: meta.TestdataVersion})
		Expect(err).NotTo(HaveOccurred())
		sub, err := submission.NewSubmissionStore(integrationDB).Create(as(ctx, owner), &submission.Submission{UserID: owner, ProblemID: item.ID, Language: "cpp", SourceCode: "int main(){}"})
		Expect(err).NotTo(HaveOccurred())
		_, _, err = writer.SaveTestdata(as(ctx, owner), item.ID, makePackage(map[string]string{"1.in": "2\n", "1.out": "2\n"}), "diff")
		Expect(err).NotTo(HaveOccurred())
		meta, err = packages.Meta(as(ctx, owner), item.ID)
		Expect(err).NotTo(HaveOccurred())
		second, err := packages.Publish(as(ctx, owner), item.ID, PublishInput{Revision: meta.PackageRevision, ArtifactVersion: meta.TestdataVersion})
		Expect(err).NotTo(HaveOccurred())
		Expect(second.Version).To(Equal(2))
		job, err := judge.NewJudgeJobStore(integrationDB).Claim(ctx, "worker", time.Minute)
		Expect(err).NotTo(HaveOccurred())
		Expect(job.SubmissionID).To(Equal(sub.ID))
		Expect(job.ProblemVersion).To(Equal(1))
		Expect(job.DomainID).To(Equal(domain.OfficialID))
	...
	It("keeps contest slots pinned across rearrangement and restores versions on cancelled rejudging", func(ctx SpecContext) {
		publish := func(answer string) *Release {
			_, _, err := writer.SaveTestdata(as(ctx, owner), item.ID, makePackage(map[string]string{"1.in": "1\n", "1.out": answer}), "diff")
			Expect(err).NotTo(HaveOccurred())
			meta, err := packages.Meta(as(ctx, owner), item.ID)
			Expect(err).NotTo(HaveOccurred())
		...
		event, err := contests.Create(as(ctx, owner), owner, &contest.PersistInput{Title: "Pinned", Rule: "icpc", Visibility: "private", Feedback: "full", BeginAt: time.Now().Add(-time.Hour), EndAt: time.Now().Add(time.Hour)})
		Expect(err).NotTo(HaveOccurred())
		entries := []contest.ProblemEntry{{ProblemID: item.ID, Label: "A", Points: 100}}
		...
		sub, err := submissions.Create(as(ctx, owner), &submission.Submission{UserID: owner, ProblemID: item.ID, ContestID: &event.ID, Language: "cpp", SourceCode: "int main(){}"})
		Expect(err).NotTo(HaveOccurred())
		Expect(sub.ProblemVersion).To(Equal(1))
		_, err = writer.Update(as(ctx, owner), item.ID, &problem.UpdateInput{CreateInput: problem.CreateInput{Title: "New title", StatementMD: "New statement", Visibility: "private", TimeLimitMs: 2000, MemoryLimitKb: 65536}})
		Expect(err).NotTo(HaveOccurred())
		publish("2\n")
		Expect(contests.SetProblems(as(ctx, owner), event.ID, entries)).To(Succeed())
		...
		Expect(contests.UseProblemVersion(as(ctx, editor), event.ID, item.ID, 2, 1)).To(MatchError(contest.ErrForbidden))
		Expect(contests.UseProblemVersion(as(ctx, owner), event.ID, item.ID, 2, 1)).To(Succeed())
		Expect(contests.UseProblemVersion(as(ctx, owner), event.ID, item.ID, 1, 1)).To(MatchError(contest.ErrVersionConflict))
		pinned, err = contests.Problem(ctx, event.ID, "A")
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(err).NotTo(HaveOccurred())
		batch, err := submissions.CreateRejudging(as(ctx, owner), submission.RejudgeSelector{ContestID: event.ID}, owner)
		Expect(err).NotTo(HaveOccurred())
		current, err := submissions.Get(as(ctx, owner), sub.ID, submission.Viewer{UserID: owner})
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(writer.Delete(as(ctx, owner), item.ID)).To(MatchError(problem.ErrReferenced))
	})
	It("searches working metadata without exposing it in the public catalogue", func(ctx SpecContext) {
		_, err := writer.Update(as(ctx, owner), item.ID, &problem.UpdateInput{CreateInput: problem.CreateInput{Title: "Unpublished search", StatementMD: "Work", Visibility: "public", TimeLimitMs: 1000, MemoryLimitKb: 65536, Tags: []string{"working-tag"}}})
		Expect(err).NotTo(HaveOccurred())
		found, total, err := reader.List(as(ctx, owner), problem.Filters{Workspace: true, ViewerID: owner, Keyword: "Unpublished", Tag: "working-tag"})
		Expect(err).NotTo(HaveOccurred())
		Expect(total).To(Equal(1))
		Expect(found[0].Title).To(Equal("Unpublished search"))
		Expect(found[0].Tags).To(ConsistOf("working-tag"))
		found, total, err = reader.List(ctx, problem.Filters{Visibility: "public"})
		Expect(err).NotTo(HaveOccurred())
		Expect(total).To(BeZero())
		Expect(found).To(BeEmpty())
	})
	It("keeps overview title edits aligned with the selected structured statement", func(ctx SpecContext) {
		_, err := packages.SaveStatement(as(ctx, owner), Statement{ProblemID: item.ID, Language: "zh", Name: "Statement name", Legend: "Body"})
		Expect(err).NotTo(HaveOccurred())
		_, err = writer.Update(as(ctx, owner), item.ID, &problem.UpdateInput{CreateInput: problem.CreateInput{Title: "Overview name", Visibility: "private", TimeLimitMs: 1000, MemoryLimitKb: 262144}})
		Expect(err).NotTo(HaveOccurred())
		_, _, err = writer.SaveTestdata(as(ctx, owner), item.ID, makePackage(map[string]string{"1.in": "1\n", "1.out": "1\n"}), "diff")
		Expect(err).NotTo(HaveOccurred())
		meta, err := packages.Meta(as(ctx, owner), item.ID)
		Expect(err).NotTo(HaveOccurred())
		_, err = packages.Publish(as(ctx, owner), item.ID, PublishInput{Revision: meta.PackageRevision, ArtifactVersion: meta.TestdataVersion})
		Expect(err).NotTo(HaveOccurred())
