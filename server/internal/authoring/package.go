...
func (p *TestdataPublisher) Publish(problemID string, archive []byte) (*PackageUpload, error) {
	if !artifactScopeRe.MatchString(problemID) {
		return nil, invalid("problem ID is not a safe artifact path segment")
	}
...
func (p *TestdataPublisher) artifactTarget(problemID, digest string) (string, string, error) {
	...
	if err != nil || relative != filepath.Join(problemID, digest) {
		return "", "", invalid("artifact path escapes the testdata root")
	}
...
func extractPackage(archive []byte, dir string) (int, bool, error) {
	...
	if err != nil {
		return 0, false, invalid("build package is not a readable zip archive")
	}
	if len(reader.File) > maxPackageEntries {
		return 0, false, invalid("build package contains too many entries")
	}
	...
	for _, entry := range reader.File {
		...
		if name != entry.Name {
			return 0, false, invalid("build package entries must not be nested: " + entry.Name)
		}
		if entry.UncompressedSize64 > uint64(maxPackageFileBytes) {
			return 0, false, invalid("build package entry exceeds the per-file limit: " + name)
		}
		if entry.UncompressedSize64 > declaredLimit-declaredBytes {
			return 0, false, invalid("build package exceeds the aggregate uncompressed size limit")
		}
		...
		case name == CheckerFileName:
			checker = entry
		default:
			return 0, false, invalid("unexpected build package entry: " + name)
		}
	}

	if highest == 0 {
		return 0, false, invalid("build package contains no tests")
	}
	...
	for index := 1; index <= highest; index++ {
		...
		if !hasInput || !hasAnswer {
			return 0, false, invalid(fmt.Sprintf("build package is missing test %d", index))
		}
	...
	if len(inputs) != highest || len(answers) != highest {
		return 0, false, invalid("build package test indexes must be contiguous from 1")
	}
	if checker != nil {
...
func copyPackageEntry(source io.Reader, destination io.Writer, entryName string, remainingBytes *int64) error {
	...
	if extra != 0 {
		if aggregateLimited {
			return invalid("build package exceeds the aggregate uncompressed size limit")
		}
		return invalid("build package entry exceeds the per-file limit: " + entryName)
	}
