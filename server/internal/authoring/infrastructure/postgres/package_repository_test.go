...
var _ = Describe("Authoring stores against PostgreSQL", func() {
	...
	BeforeEach(func(ctx SpecContext) {
		...
		user, err := identitypg.NewUserRepository(integrationDB).Create(ctx, "setter", "s@t.local", "fixture")
		Expect(err).NotTo(HaveOccurred())
		authorID = user.ID
	...
	It("bumps the package revision on every edit", func(spec SpecContext) {
		ctx := tenancydomain.WithScope(spec, tenancydomain.Scope{Domain: tenancydomain.Domain{ID: tenancydomain.OfficialID}, UserID: authorID})
		...
		_, err = packages.SaveStatement(ctx, authoringdomain.Statement{
			ProblemID: problemID, Language: "zh", Name: "求和", Legend: "给定 n 个整数",
			InputFormat: "一行", OutputFormat: "一行",
		})
		Expect(err).NotTo(HaveOccurred())

		_, err = packages.SaveFile(ctx, authoringdomain.File{
			ProblemID: problemID, Kind: authoringdomain.KindSolution, Name: "std", Language: "cpp",
			SourceCode: "int main(){}", IsActive: true, ExpectedVerdict: "Accepted",
		})
		Expect(err).NotTo(HaveOccurred())

		_, err = packages.CreateTest(ctx, authoringdomain.Test{
			ProblemID: problemID, Source: authoringdomain.TestManual, InputData: strings.Repeat("数据", 400), IsSample: true,
		})
		Expect(err).NotTo(HaveOccurred())
		...
		// Workspace previews stay bounded, while the sealed build snapshot retains
		// full source and Unicode input. Truncation is in characters, not bytes.
		files, err := packages.Files(ctx, problemID, false)
		Expect(err).NotTo(HaveOccurred())
		Expect(files).To(HaveLen(1))
		Expect(files[0].SourceCode).To(BeEmpty())
		previews, err := packages.Tests(ctx, problemID, false)
		Expect(err).NotTo(HaveOccurred())
		Expect(previews).To(HaveLen(1))
		Expect(previews[0].InputData).To(Equal(strings.Repeat("数据", 256)))
		snapshot, err := packages.Snapshot(ctx, problemID)
		Expect(err).NotTo(HaveOccurred())
		Expect(snapshot.MainSolution()).NotTo(BeNil())
		Expect(snapshot.MainSolution().SourceCode).To(Equal("int main(){}"))
		Expect(snapshot.Tests).To(HaveLen(1))
		Expect(snapshot.Tests[0].InputData).To(Equal(strings.Repeat("数据", 400)))
	...
	It("keeps exactly one active file per kind", func(spec SpecContext) {
		...
		for _, name := range []string{"std", "faster"} {
			_, err := packages.SaveFile(ctx, authoringdomain.File{
				ProblemID: problemID, Kind: authoringdomain.KindSolution, Name: name, Language: "cpp",
	...
	It("renumbers tests after a delete and reorders without collisions", func(spec SpecContext) {
		...
		for i := 0; i < 3; i++ {
			_, err := packages.CreateTest(ctx, authoringdomain.Test{
				ProblemID: problemID, Source: authoringdomain.TestManual, InputData: "x\n",
			})
			Expect(err).NotTo(HaveOccurred())
		}
	...
	It("runs one build at a time and requires explicit publication after success", func(spec SpecContext) {
		...
		Expect(builds.RecordPackage(ctx, claimed.ID, problemID, "worker-1", claimed.LeaseToken, authoringdomain.PackageUpload{StoragePath: "other-problem/hash", SHA256: "hash", CaseCount: 1})).
			To(MatchError(authoringdomain.ErrPackageTarget))
		Expect(builds.RecordPackage(ctx, claimed.ID,
			"00000000-0000-0000-0000-000000000000", "worker-1", claimed.LeaseToken, authoringdomain.PackageUpload{
				StoragePath: "00000000-0000-0000-0000-000000000000/hash",
				SHA256:      "hash", CaseCount: 1,
			})).
			To(MatchError(authoringdomain.ErrStaleLease))
		Expect(builds.RecordPackage(ctx, claimed.ID, problemID, "worker-1", claimed.LeaseToken, authoringdomain.PackageUpload{StoragePath: problemID + "/hash", SHA256: "hash", CaseCount: 1})).To(Succeed())

		sampleInput, sampleAnswer := "1\n", "1\n"
		Expect(builds.Complete(ctx, authoringdomain.BuildResult{
			BuildID: claimed.ID, WorkerID: "worker-1", LeaseToken: claimed.LeaseToken,
			Success: true, Stage: authoringdomain.StageDone,
			Tests: []authoringdomain.TestOutcome{{
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(settled.State).To(Equal(authoringdomain.BuildSucceeded))
		...
		meta, err = packages.Meta(ctx, problemID)
		Expect(err).NotTo(HaveOccurred())
		_, err = packages.Publish(ctx, problemID, authoringdomain.PublishInput{Revision: meta.PackageRevision, ArtifactVersion: meta.TestdataVersion})
		Expect(err).NotTo(HaveOccurred())
	...
	It("does not publish testdata for a failed build", func(spec SpecContext) {
		...
		settled, err := builds.Get(ctx, problemID, claimed.ID)
		Expect(err).NotTo(HaveOccurred())
		Expect(settled.State).To(Equal(authoringdomain.BuildFailed))
