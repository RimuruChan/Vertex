...
func FromBuilds(values []authoring.Build) []BuildResponse {
...
func buildFile(file *authoring.File) *BuildFile {
	if file == nil {
		return nil
	}
...
func (request BuildProgressRequest) Domain(buildID string) authoring.Progress {
	return authoring.Progress{
...
func (request BuildResultRequest) Domain(buildID string) authoring.BuildResult {
	result := authoring.BuildResult{
		BuildID: buildID, WorkerID: request.WorkerID, LeaseToken: request.LeaseToken,
		Success: request.Success, Stage: request.Stage, Log: request.Log,
		ErrorMessage: request.ErrorMessage,
		Tests:        make([]authoring.TestOutcome, 0, len(request.Tests)),
		Solutions:    make([]authoring.SolutionOutcome, 0, len(request.Solutions)),
	}
	for _, item := range request.Tests {
		result.Tests = append(result.Tests, authoring.TestOutcome(item))
	}
	for _, item := range request.Solutions {
		result.Solutions = append(result.Solutions, authoring.SolutionOutcome(item))
	}
	return result
}
