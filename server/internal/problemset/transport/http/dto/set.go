...
func (request SetUpsertRequest) Input() setdomain.UpsertInput {
	return setdomain.UpsertInput{
		Title: request.Title, Description: request.Description, Visibility: request.Visibility,
	}
}

func (request SetItemsRequest) Input() []setdomain.ItemInput {
	items := make([]setdomain.ItemInput, 0, len(request.Items))
	for _, entry := range request.Items {
		items = append(items, setdomain.ItemInput{ProblemID: entry.ProblemID, Note: entry.Note})
	}
	return items
}
...
func FromSets(values []setdomain.Set) []SetResponse {
...
func FromPermissions(p setdomain.Permissions) SetPermissions {
