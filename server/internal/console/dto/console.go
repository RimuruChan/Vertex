...
func FromStats(value console.Stats) StatsResponse {
	...
	for _, item := range value.VerdictBreakdown {
		response.VerdictBreakdown = append(response.VerdictBreakdown,
			VerdictCountResponse{Verdict: item.Verdict, Count: item.Count})
	}
	return response
}
...
func (request AccountUpdateRequest) Update() console.AccountUpdate {
	return console.AccountUpdate{
		Role: request.Role, Rating: request.Rating,
		Disabled: request.Disabled, Reason: request.Reason,
	}
}

func FromAccount(value console.AccountSummary) AccountResponse {
...
func FromAccounts(values []console.AccountSummary) []AccountResponse {
...
func FromTag(value console.Tag) TagCatalogResponse {
	return TagCatalogResponse{ID: value.ID, Name: value.Name, ProblemCount: value.ProblemCount}
}

func FromTags(values []console.Tag) []TagCatalogResponse {
...
func (request AnnouncementUpsertRequest) Input() console.AnnouncementInput {
	published := true
	if request.Published != nil {
		published = *request.Published
	}
	return console.AnnouncementInput{
...
func FromAnnouncements(values []console.Announcement) []AnnouncementResponse {
