...
var _ = Describe("Public IDs", func() {
	...
	It("keeps numeric references stable across updates and never reuses deleted numbers", func() {
		...
		Expect(db.Pool.QueryRowContext(ctx, "INSERT INTO problems(domain_id,title,owner_id) VALUES ('00000000-0000-4000-8000-000000000001'::uuid,'public-id check',$1)RETURNING id, public_id", owner).Scan(&firstID, &firstRef)).To(Succeed())
		store := references.NewResolver(db)
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(db.Pool.QueryRowContext(ctx, "INSERT INTO problems(domain_id,title,owner_id) VALUES ('00000000-0000-4000-8000-000000000001'::uuid,'next public-id check',$1)RETURNING public_id", owner).Scan(&nextRef)).To(Succeed())
		Expect(nextRef).NotTo(Equal(firstRef))
		_, err = store.Resolve(ctx, "problems", firstRef)
		Expect(err).To(MatchError(resourceid.ErrNotFound))
