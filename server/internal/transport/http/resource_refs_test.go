...
func (s *referenceStub) Resolve(_ context.Context, kind, ref string) (string, error) {
	...
	if ref == "404" {
		return "", resourceid.ErrNotFound
	}
	return kind + "-uuid", nil
}

var _ = Describe("Explicit public resource references", func() {
	It("resolves contest numbers and filters while leaving scoped labels to contest policy", func() {
		resolver := &referenceStub{}
		router := gin.New()
		router.GET("/api/contests/:id/problems/:problemId", ResourceReferences(resolver), httpx.NumberParam("contests", "id"), httpx.NumberQuery("problems", "problem"), func(c *gin.Context) {
			c.String(200, "%s/%s/%s", httpx.ResourceID(c, "id"), c.Param("problemId"), httpx.ResourceQuery(c, "problem"))
		})
	...
	It("rejects UUIDs at the public resource boundary", func() {
		resolver := &referenceStub{}
		router := gin.New()
		router.GET("/api/problems/:id", ResourceReferences(resolver), httpx.NumberParam("problems", "id"), func(c *gin.Context) { c.String(200, "%s", c.Param("id")) })
	...
	It("does not resolve protected resources before authentication succeeds", func() {
		resolver := &referenceStub{}
		router := gin.New()
		router.GET("/api/admin/problems/:id", func(c *gin.Context) { c.AbortWithStatus(401) }, ResourceReferences(resolver), func(c *gin.Context) { c.Status(200) })
		response := httptest.NewRecorder()
	...
	It("returns a generic not-found response for an absent public number", func() {
		router := gin.New()
		router.GET("/api/problems/:id", ResourceReferences(&referenceStub{}), httpx.NumberParam("problems", "id"), func(c *gin.Context) { Fail(fmt.Sprintf("handler reached for %s", c.Param("id"))) })
		response := httptest.NewRecorder()
