...
import (
	...
	dto "github.com/RimuruChan/Vertex/server/internal/identity/transport/http/dto"
...
var _ = Describe("AuthHandler", func() {
	...
	It("does not erase a concurrently rotated cookie after a refresh rejection", func() {
		service := &fakeAuthService{refreshErr: identitydomain.ErrUnauthorized}
		handler := NewAuthHandler(service, AuthCookieConfig{Lifetime: time.Hour}, AuthRateLimits{})
		router := gin.New()
		router.POST("/refresh", handler.Refresh)
		response := httptest.NewRecorder()
	...
	It("maps invalid credentials without leaking persistence errors", func() {
		service := &fakeAuthService{loginErr: identitydomain.ErrInvalidCredentials}
	...
	It("isolates registration limits by the TCP client", func() {
		service := &fakeAuthService{registerErr: identitydomain.ErrUsernameTaken}
		limiter := ratelimit.New(16)
		handler := NewAuthHandler(service, AuthCookieConfig{}, AuthRateLimits{
...
type fakeAuthService struct {
	registerErr    error
	registerCalls  int
	loginResult    *identityapp.Result
	loginErr       error
	refreshResult  *identityapp.Result
	refreshErr     error
	refreshedToken string
	loginCalls     int
}

func (f *fakeAuthService) Register(context.Context, string, string, string) (*identityapp.Result, error) {
	f.registerCalls++
	return nil, f.registerErr
}

func (f *fakeAuthService) Login(context.Context, string, string) (*identityapp.Result, error) {
	f.loginCalls++
	return f.loginResult, f.loginErr
}

func (f *fakeAuthService) Refresh(_ context.Context, token string) (*identityapp.Result, error) {
	f.refreshedToken = token
	return f.refreshResult, f.refreshErr
}

func (f *fakeAuthService) Logout(context.Context, *identityapp.Identity) error { return nil }

func (f *fakeAuthService) LogoutAll(context.Context, *identityapp.Identity) error { return nil }

func authResult(access, refresh string) *identityapp.Result {
	return &identityapp.Result{
		AccessToken: access, RefreshToken: refresh, ExpiresIn: 900,
		User: &identitydomain.User{ID: "user-1", Username: "alice", Email: "alice@example.com", Role: "user"},
	}
}
