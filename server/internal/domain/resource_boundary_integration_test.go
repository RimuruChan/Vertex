...
var _ = Describe("Resource domain boundaries against PostgreSQL", func() {
	...
	It("scopes root lists, counts, tags, details and writes even for administrators", func(spec SpecContext) {
		...
		editorial, err := editorials.Create(scoped, owner, content.EditorialInput{ProblemID: foreign.ID, Title: "Private solution", Visibility: "public", Status: "published"})
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(err).NotTo(HaveOccurred())
		_, err = posts.Get(ctx, post.ID)
		Expect(err).To(MatchError(content.ErrNotFound))
		_, err = posts.Update(ctx, post.ID, "Tampered")
		Expect(err).To(MatchError(content.ErrNotFound))
		list, err := posts.ListByProblem(ctx, foreign.ID)
		Expect(err).NotTo(HaveOccurred())
