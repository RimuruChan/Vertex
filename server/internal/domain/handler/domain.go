...
func filters(c *gin.Context) domain.Filters {
	...
	return domain.Filters{Keyword: c.Query("keyword"), Limit: size, Offset: (page - 1) * size}
}
...
func (h *Handler) Create(c *gin.Context) {
	...
	value, err := h.service.Create(c.Request.Context(), actor(c), domain.CreateInput{Slug: r.Slug, Name: r.Name, Description: r.Description, Visibility: r.Visibility, JoinPolicy: r.JoinPolicy})
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) Update(c *gin.Context) {
	...
	value, err := h.service.Update(c.Request.Context(), slug(c), actor(c), domain.UpdateInput{Name: r.Name, Description: r.Description, Visibility: r.Visibility, JoinPolicy: r.JoinPolicy})
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) SetMember(c *gin.Context) {
	...
	if bind(c, &r) {
		status(c, h.service.SetMember(c.Request.Context(), slug(c), actor(c), domain.MemberInput{Username: c.Param("username"), RoleKey: r.RoleKey, Status: r.Status}))
	}
}
...
func (h *Handler) SaveRole(c *gin.Context) {
	...
	if bind(c, &r) {
		status(c, h.service.SaveRole(c.Request.Context(), slug(c), actor(c), domain.RoleInput{Key: c.Param("role"), Name: r.Name, Permissions: r.Permissions}))
	}
}
