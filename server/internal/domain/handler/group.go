...
func (h *Handler) CreateGroup(c *gin.Context) {
	...
	value, err := h.service.CreateGroup(c.Request.Context(), slug(c), actor(c), domain.GroupInput{Name: r.Name, Description: r.Description, OwnerUsername: r.OwnerUsername})
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) UpdateGroup(c *gin.Context) {
	var r dto.GroupRequest
	if bind(c, &r) {
		status(c, h.service.UpdateGroup(c.Request.Context(), slug(c), actor(c), c.Param("group"), domain.GroupInput{Name: r.Name, Description: r.Description}))
	}
}
