...
- 题单：创建、分别保存设置/编排、用户协作/转让和按权限过滤的进度展示；比赛提供创建/编排、报名、题面、示例榜单与澄清提问。完整 group 继承与治理 mock 仍待接入。
...
域创建/成员/角色/group 治理、完整资源协作与复制的 mock 仍待补齐；未实现的接口明确报错，不回退真实网络请求。切换演示身份只影响 mock 会话，不改变正式后端权限，也不把域角色写成账号的全局角色。
