...
- 统计与反馈：公开个人统计只计本域已发布公开练习；题目计数只在练习评测完成后变化。禁反馈比赛的详情/进度/状态筛选统一按可见状态处理，列表不附带源码。模拟榜单由实际模拟报名与提交推导，支持 ICPC/IOI/OI、封榜与公开解榜，不再填入固定人员或分数。
- 社区：题解与讨论继承父题目权限，作者编辑和治理删除分开；题目/题解均可线程回复，防剧透同时限制正文、点赞与讨论，mock 点赞按独立账号保存。
- 域：左上角品牌/当前域入口打开独立选择面板，主导航保持单行。面板支持名称/标识搜索、当前域标记与身份说明，提供域目录入口；仅展示可进入的域，归档域明确只读。关闭不导航，切换只保留栏目，不携带上一域题号、比赛号或筛选参数。同号资源使用独立 UUID 和数据图，旧浏览器数据保留在官方域。
- 题单：创建、分别保存设置/编排、协作/转让和按权限过滤的进度展示。
- 比赛：协作列表只搜索、创建和进入；私有比赛创建后直接进入数字详情。详情的设置/编排/协作分别保存、保留另一分区草稿，题号/分值/颜色及固定版本不随排序重置。支持报名、题面、示例榜单与澄清，已有参赛历史不能删除；编辑协作者开赛后只读。
- 报名设置：owner/域资源管理者可分别配置自助报名和开赛后报名，默认只允许赛前自助报名。页面按实际窗口显示报名、截止或关闭状态；mock 同步时间/密码/资格规则，关闭开关不会清除已有报名。
- 协作：题目、比赛、题单共用正常协作面板，显示 owner 用户名、直接授权和 group 来源。mock 按有效域成员与当前群组关系动态继承，个人撤权不抵消组授权；比赛角色可以组合，赛务名单只读展示有效人员。
...
- 域内容治理：域设置内的标签/公告使用正常列表与详情，域 owner/资源管理者不需要站点管理员身份。标签重命名/合并/删除同步工作副本，历史发布快照保留；公告由详情明确保存发布状态，草稿不进入公开接口。公告有域内数字地址和分页搜索，页脚提供公开入口。站点后台只保留账号与系统状态。
...
- 审阅与复制：reader 可只读查看完整程序与测试定义；editor 可编辑/构建但不能发布或管理协作。工作台「复制与来源」选择已发布版本与目标域，生成独立未发布草稿并保留来源，不携带后续草稿修改或源协作者。
