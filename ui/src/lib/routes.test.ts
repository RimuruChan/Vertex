...
describe('public routes', () => {
  it('keeps contest submissions inside the contest while preserving practice routes', () => {
    expect(submissionHref({ id: '25' })).toBe('/submissions/25')
    expect(submissionHref({ id: '25', contestId: '3' })).toBe('/contests/3/submissions/25')
    expect(submissionHref({ id: '25', contestId: '3' })).toBe('/contests/3/submissions/25')
  })
  it('uses public numbers for practice and preserves the contest scope', () => {
    expect(problemHref({ problemId: '1000' })).toBe('/problems/1000')
    expect(
      problemHref({
        problemId: '1000',
  ...
  it('does not canonicalize a stale response from a previously opened resource', () => {
    ...
    expect(matchesReference('old-uuid', current)).toBe(false)
