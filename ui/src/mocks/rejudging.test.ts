...
import type { DtoRejudgingResponse, DtoSubmissionResponse } from './models'
...
describe('contest-scoped mock rejudging', () => {
  const setup = () => {
    ...
    clock += 5000
    api.handle({ method: 'GET', path: `/api/domains/official/submissions/${submission.id}` })
  ...
  it('lets jury queue and cancel before leasing while observers remain read-only', () => {
    ...
    const batch = api.handle({
      method: 'POST',
      path: '/api/domains/official/admin/rejudgings',
    ...
    expect(
      api.handle({ method: 'GET', path: `/api/domains/official/admin/rejudgings/${batch.id}` }),
    ).toHaveProperty('total', 1)
    expect(() =>
      api.handle({
        method: 'POST',
        path: `/api/domains/official/admin/rejudgings/${batch.id}/cancel`,
      }),
    ).toThrow('重测权限')
    api.state.user = { ...juryUser }
    api.handle({
  ...
  it('reports simulated changes without running code or crossing another contest', () => {
    ...
    const changes = api.handle({
      method: 'GET',
      path: `/api/domains/official/admin/rejudgings/${batch.id}/changes`,
  ...
  it('ignores saved batches whose contest no longer exists', () => {
    ...
    expect(api.handle({ method: 'GET', path: '/api/domains/official/admin/rejudgings' })).toEqual({
      items: [],
      total: 0,
    })
    expect(() =>
